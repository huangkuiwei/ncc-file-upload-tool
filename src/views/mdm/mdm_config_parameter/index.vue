<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="提示颜色" align="center" prop="colour" show-overflow-tooltip>
        <template slot-scope="scope">
          <div style="text-align: center" v-if="scope.row.colour">
            <span :style="{ background: scope.row.colour, width: '20px', height: '20px', display: 'inline-block' }" />
          </div>
...
import {
  addMdm_config_parameter,
  delMdm_config_parameter,
  pageMdm_config_parameter,
  updateMdm_config_parameter,
} from '@/api/mdm/mdm_config_parameter';
import Edit from '@/views/mdm/mdm_config_parameter/edit.vue';
import request from '@/utils/request';

export default {
  name: 'Mdm_config_parameter',
  ...
  methods: {
    ...
    pageMdm_config_parameter() {
      ...
      this.loading = true;
      pageMdm_config_parameter(this.conPO).then((response) => {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          if (this.dataPO.id != null) {
            updateMdm_config_parameter({
              ...this.dataPO,
              colour: this.$refs['form'].color === 0 ? null : this.dataPO.colour,
    ...
    handleDelete() {
      ...
      this.$confirm('是否确认删除所选择的数据？', '警告', {
        ...
        .then(function () {
          return delMdm_config_parameter(id);
        })
        .then(() => {
          this.tableKey = id;
          this.handleQuery();
    ...
    handleImport(event) {
      ...
      request({
        url: '/mdm/mdm_config_parameter/importData',
        method: 'post',
        data: formData,
        headers: {
